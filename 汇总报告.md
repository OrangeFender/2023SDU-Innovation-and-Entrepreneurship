...
### 1.2 实现效果
经过层层改进，终于实现了时间和存储的权衡，最终在`10小时内`找到了`56bit`的碰撞：
~~~
...
我们的目的是找到环与起始路径相切的点，即为碰撞。

建立两个变量，从相同起点开始，迭代更新自己为自己的值的哈希值，一个步一次迭代一次，另一个一步迭代两次，则经过x次哈希和经过2x次哈希后，两个重新回到了圆环上的某个相同点，假设该点举例碰撞相切点为$y$,n为正整数，我们有如下关系式：
$$x=a+y$$
$$2x=a+nb+y$$
...
#### 4.2.2 时间对比
实现结果并不理想，甚至不如未使用SIMD优化后的程序，主要原因是AVX指令要求读取数据必须内存对齐（四字对齐），而消息扩展阶段循环步长为三个字，读取的其他数据间隔也不同，因此读取过程非常浪费时间。
运行设备CPU为AMD Ryzen 5 5600U with Radeon Graphics            2.30 GHz
||自己的实现|openssl|
|---|---|---|
|时间（毫秒）|156|112|
