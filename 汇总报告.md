...
####  4.1.2 sm4实现
sm4实现分为密钥扩展、合成置换T(非线性变换τ和线性变换L)
...
### 9.2 实现过程中的问题
目前最大的问题在于没有找到相关指令的具体文档，导致函数返回的类型不同难以对接。同时从vscode粘到linux文本编辑器时也要注意中文空格或者符号的转变
