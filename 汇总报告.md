<img width="747" alt="image" src="https://github.com/OrangeFender/2023SDU-Innovation-and-Entrepreneurship/assets/138235459/cd9e5b5e-0f1f-42e5-9aab-6c11ccbfeff0">## Project1
由张云帆完成，最终找到56bit碰撞。
...
#### 1.1.3 关于字典
`std::map`提供了基于红黑树的字典结构，尝试用此替代数组，可以节省空间开销，代码如下(文件birthday_attack_map):
~~~C++
int main() {
...
由张云帆完成
### 2.1 ρ攻击原理
从一个起点开始，不停的计算哈希值，结果会落入一个循环中，我们记循环的周期为 $b$ ，循环前的路径长度为 $a$ .
...
```
#### 4.1.2 unroll优化
另外指定编译器对消息扩展以及压缩都进行unroll操作。

在vs中使用`#pragma loop(hint_parallel( ))`即可。
### 4.2 实现效果
#### 4.2.1 正确性
...
#### 4.2.2 时间对比
实现结果并不理想，甚至不如未使用SIMD优化后的程序，猜测主要原因是AVX指令要求读取数据必须内存对齐（四字对齐），而消息扩展阶段循环步长为三个字，读取的其他数据间隔也不同，因此读取过程非常浪费时间。
运行设备CPU为AMD Ryzen 5 5600U with Radeon Graphics            2.30 GHz
||自己的实现|openssl|
|---|---|---|
|时间（毫秒）|150|120|
